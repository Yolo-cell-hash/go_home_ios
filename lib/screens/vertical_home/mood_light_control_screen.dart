...
/// Mood Light control screen — same UI as Smart Lighting but writes to
/// separate Firebase RTDB paths for the LED strip lights.
class MoodLightControlScreen extends StatefulWidget {
  const MoodLightControlScreen({super.key});

  @override
  State<MoodLightControlScreen> createState() => _MoodLightControlScreenState();
}

class _MoodLightControlScreenState extends State<MoodLightControlScreen> {
  ...
  // Firebase database reference — same default DB as LightControlScreen
  ...
  Future<void> _fetchFirebaseState() async {
    print('[DEBUG] MoodLightControlScreen: Fetching initial Firebase state...');
    try {
      // Fetch strip light toggle state
      final lightSnapshot = await _dbRef.child('strip-lights').get();
      if (lightSnapshot.exists) {
        final lightValue = lightSnapshot.value as bool?;
        setState(() {
          isLightOn = lightValue ?? false;
        });
        print('[DEBUG] MoodLightControlScreen: Light state = $isLightOn');
      }

      // Fetch hex color value
      final hexSnapshot = await _dbRef.child('strip-lights-hex-color').get();
      if (hexSnapshot.exists) {
        ...
        if (hexValue != null && hexValue.isNotEmpty) {
          ...
          print('[DEBUG] MoodLightControlScreen: RGB color = $hexValue');
        }
      }

      // Fetch intensity value (0-255)
      final intensitySnapshot =
          await _dbRef.child('strip-lights-intensity').get();
      if (intensitySnapshot.exists) {
        ...
        if (intensityValue != null) {
          ...
          print(
            '[DEBUG] MoodLightControlScreen: Intensity = $intValue (brightness = $brightness)',
          );
        }
      }
      ...
      print(
        '[ERROR] MoodLightControlScreen: Failed to fetch Firebase data: $e',
      );
      setState(() {
        _isLoading = false;
      });
  ...
  void _setupFirebaseListeners() {
    // Listen to strip light toggle changes
    _lightSubscription =
      ...
      if (event.snapshot.exists) {
        ...
        if (mounted && lightValue != null && lightValue != isLightOn) {
          ...
          print(
            '[DEBUG] MoodLightControlScreen: Light state updated to $isLightOn',
          );
        }
      }
    });
    ...
    _hexSubscription =
        _dbRef.child('strip-lights-hex-color').onValue.listen((event) {
      if (event.snapshot.exists) {
        ...
        if (mounted && rgbValue != null && rgbValue.isNotEmpty) {
          ...
          if (newColor != selectedColor) {
            ...
            print(
              '[DEBUG] MoodLightControlScreen: RGB color updated to $rgbValue',
            );
          }
        }
      }
    ...
    _intensitySubscription =
        _dbRef.child('strip-lights-intensity').onValue.listen((event) {
      if (event.snapshot.exists) {
        ...
        if (mounted && intensityValue != null) {
          ...
          if ((newBrightness - brightness).abs() > 0.01) {
            ...
            print(
              '[DEBUG] MoodLightControlScreen: Intensity updated to $intValue (brightness = $newBrightness)',
            );
          }
        }
      }
  ...
  // ---------------------------------------------------------------------------
  // Write helpers — direct writes to automation-flags (no preset logic)
  // ---------------------------------------------------------------------------

  /// Update strip light toggle state
  void _updateLightState(bool value) {
    _dbRef.child('strip-lights').set(value);
    print('[DEBUG] MoodLightControlScreen: strip-lights = $value');
  }

  /// Update RGB color value
  void _updateRgbColor(Color color) {
    final rgbValue = _colorToRgb(color);
    _dbRef.child('strip-lights-hex-color').set(rgbValue);
    print('[DEBUG] MoodLightControlScreen: strip-lights-hex-color = $rgbValue');
  }
  ...
  void _updateIntensity(double brightnessValue) {
    ...
    _dbRef.child('strip-lights-intensity').set(intensity);
    print(
      '[DEBUG] MoodLightControlScreen: strip-lights-intensity = $intensity',
    );
  }
  ...
  // ---------------------------------------------------------------------------
  // Exit handling — simple pop (no preset override logic for mood light)
  // ---------------------------------------------------------------------------

  void _handleExit({bool isHomeTap = false}) {
    if (!mounted) return;
            ...
            child: _isLoading
                ...
                : Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        // Title with icon and toggle - "Mood Light"
                        Row(
                          children: [
                            Container(
                              width: 50,
  ...
  /// Builds elegant control button (consistent with light_control_screen)
